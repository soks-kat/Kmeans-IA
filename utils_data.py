...
def crop_images(images, upper, lower):
    ...
    for image, top_cord, bottom_cord in zip(images, upper, lower):
        cropped_image.append(
            image[top_cord[1] : bottom_cord[1], top_cord[0] : bottom_cord[0], :]
        )
    return np.array(cropped_image, dtype=object)


def read_extended_dataset(
    root_folder="./images/", extended_gt_json="./images/gt_reduced.json", w=60, h=80
):
    """
...
def read_dataset(
    root_folder="./images/", gt_json="./test/gt.json", w=60, h=80, with_color=True
):
    """
...
def visualize_retrieval(imgs, topN, info=None, ok=None, title="", query=None):
    def add_border(color):
        return np.stack(
            [
                np.pad(imgs[i, :, :, c], 3, mode="constant", constant_values=color[c])
                for c in range(3)
            ],
            axis=2,
        )
