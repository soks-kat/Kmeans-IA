...
def TripleSigmoid_E(s, tx, ty, alfa_x, alfa_y, bx, by, be, ex, ey, angle_e):
    ...
    sR = np.hstack(
        [
            sT[:, 0].reshape((-1, 1)) * np.cos(alfa_y)
            + sT[:, 1].reshape((-1, 1)) * np.sin(alfa_y),
            -sT[:, 0].reshape((-1, 1)) * np.sin(alfa_x)
            + sT[:, 1].reshape((-1, 1)) * np.cos(alfa_x),
        ]
    )
    sRE = np.hstack(
        [
            sT[:, 0].reshape((-1, 1)) * np.cos(angle_e)
            + sT[:, 1].reshape((-1, 1)) * np.sin(angle_e),
            -sT[:, 0].reshape((-1, 1)) * np.sin(angle_e)
            + sT[:, 1].reshape((-1, 1)) * np.cos(angle_e),
        ]
    )
    ex = (ex == 0.0) + ex
    ey = (ey == 0.0) + ey
    ...
    y = 1.0 / np.hstack(
        [
            1.0 + np.exp(-sR * np.hstack([by, bx])),
            1.0
            + np.exp(
                -be
                * (
                    np.sum((sRE / np.hstack([ex, ey])) ** 2, axis=1).reshape((-1, 1))
                    - 1.0
                )
            ),
        ]
...
def RGB2Lab(Ima):
    ...
    lRGB = (fRGB <= 0.04045) * (fRGB / 12.92) + (fRGB > 0.04045) * (
        ((fRGB + 0.055) / 1.055) ** 2.4
    )
    XYZ = np.dot(M, lRGB)

    f_X2 = (XYZ[0] / Xn > 0.008856) * ((XYZ[0] / Xn) ** (1.0 / 3.0)) + (
        XYZ[0] / Xn <= 0.008856
    ) * (7.787 * (XYZ[0] / Xn) + (16.0 / 116.0))
    f_Y2 = (XYZ[1] / Yn > 0.008856) * ((XYZ[1] / Yn) ** (1.0 / 3.0)) + (
        XYZ[1] / Yn <= 0.008856
    ) * (7.787 * (XYZ[1] / Yn) + (16.0 / 116.0))
    f_Z2 = (XYZ[2] / Zn > 0.008856) * ((XYZ[2] / Zn) ** (1.0 / 3.0)) + (
        XYZ[2] / Zn <= 0.008856
    ) * (7.787 * (XYZ[2] / Zn) + (16.0 / 116.0))

    L2 = (XYZ[1] / Yn > 0.008856) * (
        (116.0 * ((XYZ[1] / Yn) ** (1.0 / 3.0))) - 16.0
    ) + (XYZ[1] / Yn <= 0.008856) * (903.3 * (XYZ[1] / Yn))
    a2 = 500.0 * (f_X2 - f_Y2)
    b2 = 200.0 * (f_Y2 - f_Z2)

    ImaLab = np.reshape(
        np.vstack((L2.flatten(), a2.flatten(), b2.flatten())).T, ImaLab.shape
    )
...
def get_color_prob(ima, positions=None, patchSize=1):
    ...
    for k in range(numChromatics):
        ...
        angle_e = np.reshape(
            parameters[k, 9, m], (npix, 1)
        )  # figure;plot(angle_e); show()
        CD[:, k] = (
            np.double(beta_e != 0.0)
            * TripleSigmoid_E(
                np.vstack((a, b)),
                tx,
                ty,
    ...
    valueAchro = np.squeeze(
        np.maximum(1.0 - np.reshape(np.sum(CD, axis=1), (npix, 1)), np.zeros((npix, 1)))
    )
    CD[:, numChromatics + 0] = valueAchro * Sigmoid(
        L, paramsAchro[0, 0], paramsAchro[0, 1]
    )
    CD[:, numChromatics + 1] = (
        valueAchro
        * Sigmoid(L, paramsAchro[1, 0], paramsAchro[1, 1])
        * Sigmoid(L, paramsAchro[2, 0], paramsAchro[2, 1])
    )
    CD[:, numChromatics + 2] = valueAchro * Sigmoid(
        L, paramsAchro[3, 0], paramsAchro[3, 1]
    )
